package CSU_Yunlu_2023.extaction.pf;

import CSU_Yunlu_2023.CSUConstants;
import CSU_Yunlu_2023.module.complex.pf.GuidelineCreator;
import CSU_Yunlu_2023.standard.CSURoadHelper;
import CSU_Yunlu_2023.standard.Ruler;
import CSU_Yunlu_2023.util.CircleQueue;
import CSU_Yunlu_2023.util.CircleStack;
...
public class ActionExtClear extends ExtAction {
	...
	private EntityID lastposition = null;
	private Boolean try_rest = false;
	...
	public ExtAction calc() {
		if(this.agentInfo.getID().getValue() == 929901060)
		{
			System.out.println(this.agentInfo.getID()+" "+this.target);
		}
		if(isStucked((Human) (agentInfo.me()))){
			result = clearWhenStuck();
			if(this.agentInfo.getID().getValue() == 929901060)
			{
				System.out.println(this.agentInfo.getID()+" 1"+this.result);
			}
			if(result!= null){
				return this;
			}
		...
		if (this.is_no_move()) {
			...
			if(this.agentInfo.getID().getValue() == 929901060)
			{
				System.out.println(this.agentInfo.getID()+" 2"+this.result);
			}
			if (result != null) {
				return this;
			}
		...
		if(this.agentInfo.getID().getValue() == 929901060)
		{
			System.out.println(this.agentInfo.getID()+" 3"+this.result);
		}
		return this;
	}
	...
	private Action Calcpath(EntityID target)
	{
		EntityID position = this.agentInfo.getPosition();
		StandardEntity standardEntity = this.worldInfo.getEntity(position);
		StandardEntity TargetEntity = this.worldInfo.getEntity(target);
		if(standardEntity instanceof  Area && TargetEntity instanceof Area)
		{
			PoliceForce policeForce = (PoliceForce) this.agentInfo.me();
			return  this.getNeighbour_Rest(policeForce,(Area) TargetEntity);
		}
		return null;
	}
	private void check_same_action() {
		if (!(result instanceof ActionMove && this.lastAction instanceof ActionMove)
			...
			this.lastposition = this.agentInfo.getPosition();
			this.try_rest = false;
			lasttime = this.agentInfo.getTime();
		} else {
			int time = this.agentInfo.getTime();
			if(this.agentInfo.getPosition().equals(this.lastposition) && result instanceof ActionMove)
			{
				if(time - lasttime > 5)
				{
					List<EntityID> path = this.pathPlanning.getResult();
					if(path != null && path.size() > 1 && !try_rest) {
						result = this.Calcpath(path.get(1));
						try_rest = true;
					}
					else
...
//						System.out.println("------------------被挡没动------------");
	...
	private Action getNeighbourPositionRescueAction(PoliceForce police, Area target) {
		double agentX = police.getX();
		double agentY = police.getY();
		StandardEntity position = this.worldInfo.getEntity(this.agentInfo.getPosition());
		Edge edge = target.getEdgeTo(position.getID());
	...
	private Action getNeighbour_Rest(PoliceForce police, Area target) {
		double agentX = police.getX();
		double agentY = police.getY();
		StandardEntity position = this.worldInfo.getEntity(this.agentInfo.getPosition());
		Edge edge = target.getEdgeTo(position.getID());
		...
		Vector2D vector = this.scaleMove(this.getVector(agentX, agentY, mid.getX(),mid.getY()));
		int clearX = (int) (mid.getX() + vector.getX());
		int clearY = (int) (mid.getY() + vector.getY());
		return new ActionMove(Lists.newArrayList(police.getPosition()), clearX, clearY);
	}
